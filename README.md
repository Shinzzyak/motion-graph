...
> **Fork / turunan.** Skill ini berasal dari [bangtutorial/bang-motion](https://github.com/bangtutorial/bang-motion)
> (MIT, © 2026 Bang Tutorial) dan dikembangkan lebih lanjut di sini.
> Yang **ditambahkan** di repo ini, bukan dari upstream: `references/verifikasi-per-detik.md`,
> `scripts/perdetik.mjs`, `scripts/lacak.mjs`, `references/config-dan-terima.md`,
> `references/pemilihan-model.md`, `references/referensi-eksternal.md`, dan langkah 4b
> di `SKILL.md`. Lisensi asli dipertahankan di [LICENSE](LICENSE).

[![Version](https://img.shields.io/badge/version-1.20.0-2f6fd6?style=flat-square)](CHANGELOG.md)
...
## Sebelum push: pindai rahasia

Repo ini publik. Jalankan pemindai sebelum setiap push:

```bash
python3 scan_secrets.py .        # exit 1 = ada temuan, jangan push
```

Pemindai mencari bentuk kredensial (token, private key, JWT), alamat email, IP
publik, jalur `/home/<user>/`, dan nama berkas sensitif. Ditempatkan di root repo
supaya bisa dipakai ulang, bukan ditulis ulang tiap kali.
