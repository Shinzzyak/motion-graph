...
Ditemukan dari satu proyek nyata: empat bug, **tiga di antaranya mustahil ditemukan
dengan melihat gambar**. Kelasnya satu — *elemen yang tidak pernah benar-benar
dimatikan*.

- `references/verifikasi-per-detik.md` — dokumen baru: empat bug beserta akarnya,
  lima aturan yang lahir darinya, cara membaca keluaran alat, dan batas yang harus
  dinyatakan. Wajib dibaca sebelum menyerahkan apa pun yang bergerak.
- `scripts/perdetik.mjs` — tumpang tindih teks lintas adegan per detik, deteksi pola
  penataan (A: adegan di-fade / B: elemen di-tween langsung), kesinambungan layar,
  durasi hidup tiap teks, skala kamera. Menangkap bug kelas "tidak pernah dimatikan".
- `scripts/lacak.mjs` — satu elemen dilacak properti-per-properti di setiap detik.
  Menangkap `fromTo` yang merender state `from` segera dan menimpa `gsap.set`
  sebelumnya (bug yang tidak punya jejak di frame mana pun).
- Aturan baru: `fromTo`/`from` di detik > 0 yang menyentuh properti yang sudah
  di-`set` WAJIB `immediateRender:false`; matikan di level yang sama dengan
  masuknya; `clip-path` habis bukan berarti elemen hilang; checker dengan daftar
  kelas tetap akan buta.
