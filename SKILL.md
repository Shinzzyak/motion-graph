...
0b. **Tulis style brief** (Hukum #3) dan tunjukkan bersama rundown. Tanpa
   style brief yang disetujui, jangan menyentuh kode. Untuk proyek >3 adegan,
   yang akan direvisi, atau yang dibandingkan dengan proyek lain, tulis brief
   itu sebagai FIELD di `project.config.json` (bentuk + batasannya di
   `references/config-dan-terima.md`) — bukan paragraf prosa. Tulis juga
   `structureFingerprint` satu baris dan bandingkan dengan proyek sebelumnya.
...
4b. **Verifikasi PER DETIK — wajib, sebelum apa pun diserahkan.** Ada satu kelas
   bug yang TIDAK punya jejak di frame mana pun: **elemen yang tidak pernah
   benar-benar dimatikan.** Tiga dari empat bug di satu proyek nyata masuk kelas
   ini, dan semuanya lolos ke hasil akhir karena tidak ada yang mengukur.
   ...
   muncul di keluaran (tanda `fromTo` menimpanya). Resep dan alasannya:
   `references/verifikasi-per-detik.md`.
...
| `references/verifikasi-per-detik.md` | SEBELUM menyerahkan: kelas bug "elemen tidak pernah dimatikan" yang tidak terlihat di frame mana pun, dan lima aturan yang lahir darinya |
...
| `references/pemilihan-model.md` | saat memilih model untuk menulis kode besar: batas output terukur per model, cara mengenali kegagalan senyap (`content` kosong + `reasoning_content` besar), dan pembagian kerja yang benar |
| `references/config-dan-terima.md` | saat proyek >3 adegan, akan direvisi, atau dibandingkan: `project.config.json` (brief sebagai field + manifest aset + rundown berurut, disalin ke `index.html`), brief revisi untuk agent berikutnya, dan daftar batas yang wajib dinyatakan apa adanya |
| `references/referensi-eksternal.md` | saat membandingkan dengan skill/repo lain (bang-motion upstream, MotionForge, MotionClone) — apa yang layak diambil dan apa yang tidak |
