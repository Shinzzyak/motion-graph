...
| **Elemen SVG terlempar keluar frame saat berputar** (roda gigi, jarum, tuas) | `transformOrigin:'600px 400px'` pada elemen SVG dibaca GSAP **relatif ke bbox elemen**, bukan ke ruang user SVG. Untuk `<circle cx=600 cy=400 r=60>` (bbox `540,340`), pivot jadi `(1140,740)` bukan `(600,400)` — `rotate(180)` menghasilkan `matrix(-1,0,0,-1,2280,1480)` alih-alih `(1200,800)`, dan elemen **bergeser 1080×680 satuan SVG (907×571 px di panggung)**. Diukur 2026-09-24: roda gigi mendarat di `x=1903` pada panggung 1920 — separuh keluar frame, hanya saat adegan terakhir, dan **tidak terlihat di frame mana pun karena saat itu kamera diam di z=1** | **`svgOrigin:'600 400'`** — koordinat ruang user SVG, tanpa satuan. Perbaiki SEMUA tween yang memutar elemen SVG, termasuk yang terlihat "cukup benar" (jarum jam bergeser 170 px tanpa ada yang sadar) |
